 ...
 */
describe(USER_COUNT + " concurrent users (" + TEAM_COUNT + " teams x " + MEMBERS_PER_TEAM + " members)", () => {
  beforeAll(async () => {
  ...
  function seed(prefix, overrides = {}) {
    ...
    for (let i = 0; i < TEAM_COUNT; i++) {
      ...
      teams.push({
        ...
        members: ["Leader " + i, "Member " + i + "b", "Member " + i + "c", "Member " + i + "d"],
  ...
  test("600 users read the event config at once and all see the same thing", async () => {
    ...
    const responses = await Promise.all(
      Array(USER_COUNT).fill().map(() => request(app).get("/api/event")),
    );
  ...
  test("600 health probes at once all report healthy", async () => {
    ...
    const responses = await Promise.all(
      Array(USER_COUNT).fill().map(() => request(app).get("/health")),
    );
  ...
  test("a team's 10-attempt budget is shared by all four members", async () => {
    ...
    const team = teams[0];
    const memberTokens = Array(MEMBERS_PER_TEAM).fill().map(() => signToken(team.id));
  ...
  test("600 logins from one venue IP are throttled to 5 a minute", async () => {
    ...
    const responses = await Promise.all(
      everyone.map((client) =>
        request(app)
          .post("/api/login")
          .send({ team_name: client.team.team_name, password: PASSWORD }),
      ),
    );
    ...
    expect(throttled).toHaveLength(USER_COUNT - 5);
    for (const res of ok) {
  ...
  test("600 unauthenticated polls are all rejected", async () => {
    ...
    const responses = await Promise.all(
      Array(USER_COUNT).fill().map(() => request(app).get("/api/team/state")),
    );
  ...
  test("admin sees 150 teams while 600 users play", async () => {
    ...
    const [adminRes, ...userResponses] = await Promise.all([
      request(app).get("/api/admin/teams").set("x-admin-secret", process.env.ADMIN_SECRET),
      ...everyone.map((client) =>
        request(app)
          .get("/api/team/state")
          .set("Authorization", "Bearer " + client.token),
  ...
  test("event gating holds for all 600 before start and after end", async () => {
    ...
    await request(app).post("/api/admin/start").set("x-admin-secret", process.env.ADMIN_SECRET);
    await request(app).post("/api/admin/end").set("x-admin-secret", process.env.ADMIN_SECRET);
  ...
  test("leaderboard ordering over 150 teams stays total while 600 users play", async () => {
    ...
    for (let i = 1; i < board.length; i++) {
      ...
      if (board[i - 1].progress === board[i].progress) {
        expect(new Date(board[i - 1].last_correct_at).getTime())
          .toBeLessThanOrEqual(new Date(board[i].last_correct_at).getTime());
      }
    }
  }, 60000);
});
